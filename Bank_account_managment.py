...
def create_account():
    ...
    name = input('Please enter an account holder name: ')
    ...
    print(f'You have created an account for {name}')


def deposit():
    """Deposit money into an account."""
    # 1. Prompt the user for the account holder's name.
    name_account_deposit = input("Which account do you want to use: ")
    # 2. Check if the account exists in the system.
    if name_account_deposit in account_holders:
        index = name_account_deposit.index(name_account_deposit)
        depos = float(input('How much do you want to deposit: '))
    # 4. Update the account's balance with the deposited amount.
        balances[index] += depos
    # 5. Log the transaction in the account's transaction history.
        transaction_histories[index].append([f'You deposited: {depos} leva.'])
    # 6. Display the updated balance to the user.
        print(f"You currently have: {balances[index]} leva.")
